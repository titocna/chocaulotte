...
class WonderPlugin_3DCarousel_List_Table extends WP_List_Table {
	...
	function get_columns()
	{
		$columns = array(
				...
				'id' => __('ID', 'wonderplugin_3dcarousel'),
				'name' => __('Name', 'wonderplugin_3dcarousel'),
				'shortcode' => __('Shortcode', 'wonderplugin_3dcarousel'),
				'phpcode' => __('PHP code', 'wonderplugin_3dcarousel'),
				'time' => __('Created', 'wonderplugin_3dcarousel')
		);
		return $columns;
	}
	...
	function column_default( $item, $column_name )
	{
		...
		switch( $column_name ) {
			case 'id':
				...
				else
				{
					$actions = array(
							...
							'view' => sprintf('<a href="?page=%s&itemid=%s">View</a>', 'wonderplugin_3dcarousel_show_item', $item['id']),
							'edit' => sprintf('<a href="?page=%s&itemid=%s">Edit</a>', 'wonderplugin_3dcarousel_edit_item', $item['id'])
					);
				}
			...
			case 'shortcode':
				return esc_attr('[wonderplugin_3dcarousel id=' . $item['id'] . ']');
			case 'phpcode':
				return esc_attr('<?php echo do_shortcode(\'[wonderplugin_3dcarousel id=' . $item['id'] . ']\'); ?>');
			default:
				return $item[ $column_name ];
		}
	}
	...
	function get_views(){
		...
		// All
		$all_url = admin_url('admin.php?page=wonderplugin_3dcarousel_show_items');
		$class = ($current == 'all' ? ' class="current"' :'');
		$views['all'] = "<a href='" . $all_url . "' " . $class . " >All (" . $this->published . ")</a>";
	
		// Published
		$publish_url = admin_url('admin.php?page=wonderplugin_3dcarousel_show_items&item_status=publish');
		$class = ($current == 'publish' ? ' class="current"' :'');
		$views['publish'] = "<a href='" . $publish_url . "' " . $class . " >Published (" . $this->published . ")</a>";
	
		// Trash
		$trash_url = admin_url('admin.php?page=wonderplugin_3dcarousel_show_items&item_status=trash');
