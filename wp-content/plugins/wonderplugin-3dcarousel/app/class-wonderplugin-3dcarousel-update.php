...
class WonderPlugin_3DCarousel_Update {
	...
	function __construct($controller) {
		...
		$this->product = '3dcarousel';
		$this->slug = 'wonderplugin-3dcarousel';
		$this->plugin = WONDERPLUGIN_3DCAROUSEL_PLUGIN;
		$this->version = WONDERPLUGIN_3DCAROUSEL_PLUGIN_VERSION;
	...
	function plugin_update_notice() {
		...
		if( $pagenow === 'update-core.php' && !empty($info))
		{
			if ( !isset($info->key_status) || $info->key_status != 'valid')
			{
				...
				{
					if (WONDERPLUGIN_3DCAROUSEL_VERSION_TYPE == 'F')
					{
						echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: To update the plugin in WordPress backend, please <a href="https://www.wonderplugin.com/wordpress-3dcarousel/order/" target="_blank">upgrade to a commercial license</a>, otherwise, you will get the <strong>Update package not available</strong> error.</div>';				
					}
					else
					{
						echo '<div class="error notice is-dismissible"><p><strong>' . $info->name . '</strong>: To update the plugin, please <a href="'. admin_url('admin.php?page=wonderplugin_3dcarousel_register') . '">enter your license key</a>, otherwise, you will get the <strong>Update package not available</strong> error.</div>';
					}
				}
			}
		}
	...
	function show_plugin_update_message( $plugin_data, $r ) {
		...
		else
		{
			if (WONDERPLUGIN_3DCAROUSEL_VERSION_TYPE == 'F')
			{
				echo '<br>To get automatic update, please <a href="https://www.wonderplugin.com/wordpress-3dcarousel/order/" target="_blank">upgrade to a commercial license</a>.';
			}
			else
			{
				echo '<br>To get automatic update, please <a href="'. admin_url('admin.php?page=wonderplugin_3dcarousel_register') . '">enter your license key</a> or <a href="https://www.wonderplugin.com/register-faq/#manualupdate" target="_blank">update the plugin manually</a>.';
			}
		}
	}
}